...
async function ProductsTableContent({
  ...
  subdomain,
  userId: _userId,
  employeePermissions: _employeePermissions,
  branches: _branches,
  ...
  const {
    ...
    hasError,
    message,
  } = await getProductsByStoreAction(identifier, limit, orderBy, page, search);
  ...
  return <ProductsTable data={products} storeId={resolvedStoreId} slug={slug} />;
}
