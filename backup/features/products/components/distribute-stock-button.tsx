...
function DistributeStockButton({
  ...
  useEffect(() => {
    ...
    if (variantStocks && variantStocks.length > 0) {
      ...
      branches.forEach((branch) => {
        const currentStock =
          branch.stock.find((stock) => stock.product_id === productId)
            ?.quantity || 0;
  ...
  const isValidDistribution =
    totalDistributed > 0 && totalDistributed <= availableStock;
    ...
    >
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <Badge
            variant={
              totalDistributed > availableStock ? "destructive" : "outline"
            }
          >
            {totalDistributed}
          </Badge>
              ...
              <div key={branch.id} className="flex items-center gap-3">
                ...
                <div className="w-20">
                  <Input
                    ...
                    onChange={(e) =>
                      handleQuantityChange(branch.id, e.target.value)
                    }
                    placeholder="0"
        ...
        <div className="text-xs text-muted-foreground">
          {t("distribute-stock.remaining-stock")}:{" "}
          {availableStock - totalDistributed}
        </div>
      </div>
    </ButtonWithPopup>
  );
