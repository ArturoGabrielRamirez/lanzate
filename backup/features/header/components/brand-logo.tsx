...
function BrandLogo({ hasText = true, className }: BrandLogoProps) {
    return (
        <Link href="/" className={cn("flex items-center gap-2 group text-primary", className)}>
            ...
            {hasText && <span className={cn("hidden md:inline-block text-xl font-bold text-foreground")}>Lanzate</span>}
        </Link>
    );
};
