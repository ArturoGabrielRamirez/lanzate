...
export type DimensionsSectionProps = {
...
export type PriceStockSectionDefaults = {
  price?: number | string;
  stock?: number | string;
};
...
export type VariantPriceDisplayProps = VariantDisplayBaseProps & {
  productPrice: number;
};
...
export type VariantSizesDisplayProps = VariantDisplayBaseProps & {
  product: VariantDimensions;
};

export type VariantStockDisplayProps = VariantDisplayBaseProps & {
  variant: VariantWithStock & {
    stocks?: (StockInfo & { branch?: { id: number; name: string } })[];
  };
};
...
export type ProductWithAllRelations = ProductWithMedia &
  ProductWithVariants & {
    categories: Category[];
  };
...
export type ProductDimensions = Pick<ProductWithVariants, "variants">;
