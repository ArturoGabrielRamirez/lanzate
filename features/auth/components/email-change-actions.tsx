...
export default function EmailChangeActions(
    { stepStatus,
        ...
        resendCooldown,
        handleResendEmails,
        onComplete }: EmailChangeActionsProps
) {
    ...
    return (
        <div className="space-y-2">
            <Button
                ...
                className={`w-full flex items-center gap-2 transition-all duration-300 ${isManuallyChecking
                    ? 'scale-95 bg-blue-50 dark:bg-blue-950 border-blue-300 dark:border-blue-700'
                    : 'hover:scale-[1.02]'
                    }`}
            >
                <RefreshCw className={`w-4 h-4 transition-transform duration-500 ${(status.loading || isManuallyChecking) ? 'animate-spin' : ''
                    }`} />
                {isManuallyChecking
                    ? 'Verificando...'
                    : status.loading
            ...
            <Button
                ...
                className={`w-full flex items-center gap-2 transition-all duration-200 ${isResending ? 'scale-95' : 'hover:scale-[1.02]'
