    ...
    return (
        <div className="fixed w-full bottom-0 px-8 z-50 flex justify-center items-center bg-background/85 backdrop-blur-[4px] rounded-lg py-4 shadow-2xl shadow-black drop-shadow-2xl md:hidden">
            ...
            <div className="flex items-center gap-3 relative z-10">
                <Button variant={pathname.includes(`/${locale}/dashboard`) ? "default" : "outline"} size="icon" asChild className={cn(pathname.includes(`/${locale}/dashboard`) && "scale-125","transition-all duration-300")}>
                    <Link href="/dashboard">
                        <Home className="size-5" />
                    </Link>
                </Button>
                <Button variant={pathname.includes(`/${locale}/sale`) ? "default" : "outline"} size="icon" asChild className={cn(pathname.includes(`/${locale}/sale`) && "scale-125","transition-all duration-300")}>
                    <Link href="/sale">
                ...
                <Button variant={pathname.includes(`/${locale}/stores`) ? "default" : "outline"} size="icon" asChild className={cn(pathname.includes(`/${locale}/stores`) && "scale-125","transition-all duration-300")}>
                    <Link href="/stores">
                        <Store className="size-5" />
                    </Link>
                </Button>
                <Button variant={pathname.includes(`/${locale}/events`) ? "default" : "outline"} size="icon" asChild className={cn(pathname.includes(`/${locale}/events`) && "scale-125","transition-all duration-300")}>
                    <Link href="/events">
                        <Calendar className="size-5" />
                    </Link>
                </Button>
                <Button variant={pathname.includes(`/${locale}/account`) ? "default" : "outline"} size="icon" asChild className={cn(pathname.includes(`/${locale}/account`) && "scale-125","transition-all duration-300")}>
                    <Link href="/account">
