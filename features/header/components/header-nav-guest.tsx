...
} from "@/features/shadcn/components";
      ...
      <NavigationMenuList>
        {NAV_MENU_ITEMS_GUEST.map((menuItem) => (
          <NavigationMenuItem key={menuItem.label}>
            <NavigationMenuTrigger
              className={cn("bg-transparent", !isHome && "text-foreground")}
            >
              {t(menuItem.label)}
            </NavigationMenuTrigger>
            <NavigationMenuContent>
                  ...
                  menuItem.items.map((item) => {
                    if (item.isGoogleAuth) {
                      return (
                        <li
                          key={item.label}
                          className="row-span-3 col-start-1 row-start-1 flex flex-col gap-2 p-2"
                        >
