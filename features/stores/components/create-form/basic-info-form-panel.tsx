 ...
 */
    ...
    const handleNameChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setCtxValues({ basic_info: { name, subdomain: slugify(name) } })
        setValue("basic_info.subdomain", slugify(name), { shouldValidate: true, shouldDirty: true })
    }, [setCtxValues, setValue])

    const handleSubdomainChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
        const subdomain = e.target.value
        setCtxValues({ basic_info: { name: basic_info?.name || "", subdomain } })
    }, [setCtxValues, basic_info])

    const handleDescriptionChange = useCallback((e: React.ChangeEvent<HTMLTextAreaElement>) => {
        const description = e.target.value
        setCtxValues({ basic_info: { name: basic_info?.name || "", subdomain: basic_info?.subdomain || "", description } })
    }, [setCtxValues, basic_info])
